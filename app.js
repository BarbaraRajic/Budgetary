...
var UIController = (function () {
  ...
  displayButtons = function (event) {
    ...
    event.currentTarget.querySelector(DOMStrings.btnEdit).style.display =
      "block";
    event.currentTarget.querySelector(DOMStrings.itemValue).style.transform =
      "translateX(" + -20 + "px)";
  ...
  return {
    ...
    addItem: function (budgetItem, type) {
      ...
      html = html.replace("%value%", formatNumber(budgetItem.value, type));
    ...
    editItem: function (selectorID, type, value) {
      ...
      // Allaw edits
      item
        .querySelector(DOMStrings.itemDescription)
        .setAttribute("contenteditable", true);
      item
        .querySelector(DOMStrings.itemValue)
        .setAttribute("contenteditable", true);

      item
        .querySelector(DOMStrings.itemDescription)
        .addEventListener("keydown", controlEditInput);
      item
        .querySelector(DOMStrings.itemValue)
        .addEventListener("keydown", controlEditInput);
      ...
      // Remove percentage
      if (type === "exp") {
        item.querySelector(DOMStrings.itemPercentage).style.display = "none";
      }
    ...
    getEdit: function (selectorID) {
      ...
      return {
        ...
        description: item.querySelector(DOMStrings.itemDescription).innerHTML,
      };
    },
    ...
    saveItemEdits: function (selectorID, type, value) {
      ...
      // Display percentage
      if (type === "exp") {
        item.querySelector(DOMStrings.itemPercentage).style.display = "block";
      }
    ...
    updateBudget: function (obj) {
      ...
      document.querySelector(DOMStrings.incValue).textContent = formatNumber(
        obj.inc,
        "inc"
      );
      document.querySelector(DOMStrings.expValue).textContent = formatNumber(
        obj.exp,
        "exp"
      );
    ...
    updatePercentages: function (percentages) {
      for (var i = 0; i < percentages.length; i++) {
        if (percentages[i] > 0) {
          document.querySelectorAll(DOMStrings.itemPercentage)[i].textContent =
            percentages[i] + "%";
        } else {
          document.querySelectorAll(DOMStrings.itemPercentage)[i].textContent =
            "%";
